...
#quantization functions applied to convolution, gru and linear layers
#quant_tensor=namedtuple('quant_tensor',['tensor','scale','zero_point'])

def calc_scale_zeropoint(min_val,max_val,num_bits=4):
        qmin=0.#range of 4 bit number
        qmax=2.**num_bits-1
        scale=(max_val - min_val)/(qmax-qmin)
        initial_zero_point=qmin-min_val/scale
        zero_point=0#does shifting
...
def quantize_tensor(x,num_bits=4,min_val=None,max_val=None):
        ...
        qmin=0.
        qmax=2.**num_bits-1.
        scale,zero_point=calc_scale_zeropoint(min_val,max_val,num_bits)
    ...
    def reset_parameters(self):
        ...
        for name, param in self.named_parameters():
            ...
            if 'weight_hh' in name:
                ...
                w,scale,zero_point=quantize_tensor(w)
                winit=w.numpy()
                init_(param, winit)
...
class GruMod_Quant(nn.Module):
    ...
    def reset_parameters(self):
        ...
        """
        '''#quantize weights and bias
        self.cudnn_gru.weight_hh_l0=quantize_tensor(self.cudnn_gru.weight_hh_l0)
        self.cudnn_gru.weight_ih_l0=quantize_tensor(self.cudnn_gru.weight_ih_l0)
        self.cudnn_gru.bias_ih_l0=quantize_tensor(self.cudnn_gru.bias_ih_l0)'''
    ...
    def forward(self, x):
        #print(x)
        #quantize inputs
        self.x,scale,zero_point=quantize_tensor(x)
        #print(self.x)
        ...
        y=dequantize(y,scale,zero_point)
        ...
        Returns:
            :torch:`Tensor`: Output of layer
        """
	    #self.x,scale,zero_point=quantize(x,num_bits=4)
        #y, hy = self.cudnn_gru.forward(x)
	    #y=dequantize(y,scale,zero_point)
        #return y
    ...
    def json(self):
        ...
        self.cudnn_gru.weight_hh_l0=dequantize(self.cudnn_gru.weight_hh_l0)
        self.cudnn_gru.weight_ih_l0=dequantize(self.cudnn_gru.weight_ih_l0)
        self.cudnn_gru.bias_ih_l0=dequantize(self.cudnn_gru.bias_ih_l0)'''

        res = OrderedDict([('type', "GruMod_Quant"),
                           ('activation', "tanh"),
                           ('gate', "sigmoid"),
                           ('size', self.size),
                           ('insize', self.insize),
...
class Convolution_Quant(nn.Module):
    ...
    def reset_parameters(self):
        ...
        """
        '''quantizing weights and bias 
        self.conv.weight.tensor,scale,zero_point=quantize_tensor(self.conv.weight.tensor)
        self.conv.bias.tensor,scale,zero_point=quantize_tensor(self.conv.bias.tensor)'''

        #Quantizing the weights and bias
    ...
    def forward(self, x):
        ...
        #dequantize output
        x=dequantize(x,scale,zero_point)
        out=dequantize(out,scale,zero_point)
    ...
    def json(self):
        ...
        Returns:
            :collections:`OrderedDict`: Structured description of layer.
        """
        '''#dequantize the weights and bias
        self.conv.weight=dequantize(self.conv.weight)
        self.conv.bias=dequantize(self.conv.bias)'''


        res = OrderedDict([("type", "convolution_Quant"),
                           ("insize", self.insize),
                           ("size", self.size),
                           ("bias", self.has_bias),
...
class GlobalNormFlipFlop_Quant(nn.Module):
    ...
    def json(self):
        ...
        """
        '''#dequantize weights and bias
        self.linear.bias=dequantize(self.linear.bias)
        self.linear.weight=dequantize(self.linear.weight)'''
    ...
    def reset_parameters(self):
        ...
        """
        '''#quantize weights and bias
        self.linear.bias=quantize_tensor(self.linear.bias)
        self.linear.weight=quantize_tensor(self.linear.weight)'''
    ...
    def forward(self, x):
        ...
        return dequantize(y,scale,zero_point)
