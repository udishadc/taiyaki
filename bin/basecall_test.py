...
def worker_init(device, modelname, chunk_size, overlap,
    ...
    qmodel = torch.quantization.quantize_dynamic(model, {nn.LSTM}, dtype=torch.qint8)
    stride = guess_model_stride(qmodel)
    ...
    def process_read_partial(read_filename, read_id, read_params):
        res = process_read(read_filename, read_id,
                           qmodel, chunk_size, overlap, read_params,
...
def process_read(
        read_filename, read_id, qmodel, chunk_size, overlap, read_params,
    ...
    with torch.no_grad():
        ...
        for some_chunks in torch.split(chunks, max_concurrent_chunks, 1):
            trans.append(qmodel(some_chunks)[:, :, :n_can_state])
...
def size(model,qmodel, label=""):
    torch.save(model.state_dict(), "temp.p")
    size=os.path.getsize("temp.p")
    print("model: ",label,' \t','Size (KB):', size/1e3)
    os.remove('temp.p')
    f=size(model,"LSTM model")
    q=size(qmodel,"Quantized model")
